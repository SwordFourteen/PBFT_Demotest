...
const inital float64 = 0.7
const topLimit float64 = 0.47
...
func NewPBFT(nodeID, addr string) *pbft { //初始化PBFT节点
	p := new(pbft)
	p.node.nodeID = nodeID
	p.node.addr = addr
	for i := 0; i < nodeCount; i++ {
		p.nodeWeight[i] = inital
	}
...
func (p *pbft) handlePrePrepare(content []byte) {
	...
	if digest := getDigest(pp.RequestMessage); digest != pp.Digest {
		...
		fmt.Printf("到发送完prepare用时%d\n", time.Now().UnixMicro()-p.messagePool[pre.Digest].Timestamp)
...
func (p *pbft) handlePrepare(content []byte) {
	...
	if _, ok := p.messagePool[pre.Digest]; !ok {
		...
		if count >= specifiedCount && !p.isCommitBordcast[pre.Digest] {
			fmt.Printf("本节点已收到至少%f节点(包括本地节点)发来的Prepare信息 ...", specifiedCount)
			//节点使用私钥对其签名
			sign := p.RsaSignWithSha256(digestByte, p.node.rsaPrivKey)
...
func (p *pbft) handleCommit(content []byte) {
	...
	if _, ok := p.prePareConfirmCount[c.Digest]; !ok {
		...
		p.updateWeight(nsuccess, p.node.nodeID) //共识成功更新信任度
		count := 0.0
		n, _ := strconv.ParseInt(c.NodeID[1:], 10, 0)
		//p.lock.Lock()
		for range p.commitConfirmCount[c.Digest] {
			count += p.nodeWeight[n]
			//fmt.Printf("本节点收到%s的prepare信息权重加%f当前权重为%f\n", p.prePareConfirmCount[pre.Digest], p.nodeWeight[n], count)
		}
		//因为主节点不会发送Prepare，所以不包含自己
		var specifiedCount float64 = float64(nodeCount) * topLimit
		p.lock.Lock()
		//如果节点至少收到了足够权重的个prepare的消息（包括自己） ,并且没有进行过commit广播，则进行commit广播
		if count >= specifiedCount && !p.isReply[c.Digest] && p.isCommitBordcast[c.Digest] {
			//本节点已收到至少2f + 1 个节点(包括本地节点)发来的Commit信息 ...
			localMessagePool = append(localMessagePool, p.messagePool[c.Digest].Message)
			p.setRepltconfirmap(c.Digest, true)
			//info := p.node.nodeID + "节点已将msgid:" + strconv.Itoa(p.messagePool[c.Digest].ID) + "存入本地消息池中,消息内容为：" + p.messagePool[c.Digest].Content
			//fmt.Println(info)

			time1 := time.Now().UnixMicro() - p.messagePool[c.Digest].Timestamp
			mes := IntToBytes(time1)
			p.isReply[c.Digest] = true
			tcpDial(mes, p.messagePool[c.Digest].ClientAddr)
...
func (p *pbft) broadcast(cmd command, content []byte) {
	id, _ := strconv.ParseInt(p.node.nodeID[1:], 10, 0)
	for i := 0; i < nodeCount; i++ {
		if i == int(id) {
			continue
		}
		message := jointMessage(cmd, content)
		str := "N" + strconv.Itoa(i)
		go tcpDial(message, nodeTable[str])
	}
}
...
//为reply池子赋值
func (p *pbft) setRepltconfirmap(val string, b bool) {
	p.isReply[val] = b
}
